            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        {
                            _context.MyExpensePlans.Add(new MyExpensePlan() { UserId = appUser.Id, ExpenseTags = t.TagName, Discription = t.Description, IsMandatory = true, CreatedDate = DateTime.UtcNow });
                        });
                        await _context.SaveChangesAsync();
